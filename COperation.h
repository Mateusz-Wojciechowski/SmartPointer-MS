...
string COperation<string>::t_sub(const vector<string> &operands) {
    ...
    if(i_pos!=I_CHAR_NOT_FOUND_VALUE){
...
string COperation<string>::t_mul(const vector<string> &operands) {
    ...
    while((i_pos = s_result.find(c_first_char, i_pos)) != I_CHAR_NOT_FOUND_VALUE){
...
string COperation<string>::t_div(const vector<string> &operands) {
    ...
    while((i_pos = s_result.find(operands[1], i_pos)) != I_CHAR_NOT_FOUND_VALUE){
...
template<>
bool COperation<bool>::t_add(const vector<bool> &operands){
    return operands[0] || operands[1];
}

template<>
bool COperation<bool>::t_mul(const vector<bool> &operands){
    return operands[0] & operands[1];
}

template<>
bool COperation<bool>::t_sub(const vector<bool> &operands){
    return (operands[0] || operands[1]) || operands[0];
}

template <>
bool COperation<bool>::t_div(const std::vector<bool> &operands){
    return (operands[0] & operands[1]) || operands[0];
}

template <>
bool COperation<bool>::t_sin(const std::vector<bool> &operands){
    return operands[0];
}

template <>
bool COperation<bool>::t_cos(const std::vector<bool> &operands){
    return !operands[0];
}
