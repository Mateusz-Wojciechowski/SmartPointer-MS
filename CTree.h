...
public:
    ...
    CTree operator+(CTree<T> &&c_other);
    CTree& operator=(const CTree& c_other);
    CTree& operator=(CTree<T> &&c_other);
...
template<typename T>
CTree<T>::CTree(CTree<T> &&c_other){
    cout << "Move" << endl;
    root = c_other.root;
    operations = move(c_other.operations);
    b_was_changed = c_other.b_was_changed;
    b_wrong_arguments = c_other.b_wrong_arguments;

    c_other.root = NULL;
    c_other.b_was_changed = false;
    c_other.b_wrong_arguments = false;
}
...
CNode<T>* CTree<T>::createTree(string s_prefix, int &i_index){
    ...
    if (bIsOperator(s_expression)) {
        ...
        if(s_expression==S_SIN || s_expression==S_COS){
            throw std::invalid_argument(S_WRONG_OPP);
        }
...
template<typename T>
CTree<T>& CTree<T>::operator=(CTree<T> &&c_other) {
    if (this != &c_other){
        delete root;

        cout << "Move =" << endl;
        root = c_other.root;
        operations = std::move(c_other.operations);
        b_was_changed = c_other.b_was_changed;
        b_wrong_arguments = c_other.b_wrong_arguments;
...
CTree<T> CTree<T>::operator+(CTree<T> &&c_other) {
    ...
    c_result.vJoinTrees(c_other.root);
    c_other.root = NULL;
...
bool CTree<string>::bIsTTypevalue(string s_value){
    int i_first_quote = s_value.find(C_PARENTHESES);
    if(i_first_quote==I_OP_NOT_FOUND){
        return false;
    }

    int i_second_quote = s_value.find(C_PARENTHESES, i_first_quote + 1);
    if(i_second_quote==I_OP_NOT_FOUND){
        return false;
    }
...
template <>
bool CTree<bool>::bIsTTypevalue(string s_value){
    if(s_value==S_VALUE_TRUE) return true;
    else if(s_value==S_VALUE_FALSE) return true;
    else return false;
}
...
template<>
bool CTree<bool>::getValueFromString(string s_value) {
    if(s_value==S_VALUE_TRUE) return true;
    else return false;
}
...
template<>
CNode<bool>* CTree<bool>::createDefaultNode(){
    CNode<bool>* c_node = new CNode<bool>(S_VALUE_TRUE);
    c_node->setTValue(getValueFromString(S_VALUE_TRUE));
    return c_node;
}
